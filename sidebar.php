	...
	if ( is_archive() || is_single() ) :
		...
		<div class="bg-faded sidebar-nav">
			...
			if ( $recentposts_query->have_posts() ) :
				$output .= '<li><h3>' . esc_html__( 'Recent Posts', 'dxndre' ) . '</h3></li>';
				while ( $recentposts_query->have_posts() ) :
					...
					if ( $month !== $month_check ) :
						$output .= '<a href="' . esc_url( get_month_link( get_the_date( 'Y' ), get_the_date( 'm' ) ) ) . '" title="' . esc_attr( get_the_date( 'F, Y' ) ) . '">' . esc_html( $month ) . '</a>';
							...
							$output .= '<h4><a href="' . esc_url( get_permalink() ) . '" title="' . sprintf( esc_attr__( 'Permalink to %s', 'dxndre' ), the_title_attribute( array( 'echo' => false ) ) ) . '" rel="bookmark">' . esc_html( get_the_title() ) . '</a></h4>';
							$output .= '</li>';
					endwhile;
						endif;
					wp_reset_postdata();
			...
			?>
				<br />
				<ul class="categories">
					<li><h3><?php esc_html_e( 'Categories', 'dxndre' ); ?></h3></li>
				<?php
				...
				if ( ! is_author() ) :
					?>
							<li>&nbsp;</li>
							<li><a href="<?php the_permalink( get_option( 'page_for_posts' ) ); ?>" class="btn btn-outline-secondary"><?php esc_html_e( 'more', 'dxndre' ); ?></a></li>
					<?php
					endif;
				?>
				</ul>
